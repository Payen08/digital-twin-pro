...
const Scene2DRenderer = ({ objects, selectedId, selectedIds, viewMode, transformMode, onTransformEnd, onSelect }) => {
    ...
    return (
        <group>
            {objects.filter(obj => obj.visible && obj.type !== 'group').map(obj => {
...
const App = () => {
    const initialObjects = [{ id: '0', type: 'floor', name: '基础地面', position: [0, -0.11, 0], rotation: [0, 0, 0], scale: [30, 0.2, 30], color: '#151515', visible: true, locked: true, isBaseMap: true }];
    ...
    const [floors, setFloors] = useState([
        { id: 'default', name: '默认场景', description: '默认场景', baseMapId: null, objects: [], isDefault: true }
    ]);
    ...
    }, [toolMode, isEditingPoints, selectedIds]);
    ...
    const handleDrawFinish = (data) => {
        let newObj = { id: uuidv4(), color: '#8b5cf6', visible: true, opacity: 1, position: [0, 0, 0], rotation: [0, 0, 0], scale: [1, 1, 1], ...data };
    ...
    const handleDragEnd = () => {
        console.log('handleDragEnd', { dragOffset, selectedIds, isDragging });
    ...
    // 计算用于显示的临时对象列表（包含拖动偏移）
    const displayObjects = useMemo(() => {
        if (!isDragging || !dragOffset) return objects;
    ...
    }, [objects, isDragging, dragOffset, selectedIds]);
                                ...
                                onClick={async () => {
                                    ...
                                    if (editingFloor.isNew) {
                                        ...
                                        try {
                                            ...
                                            // 2. 解析地图数据
                                            const { baseMap, entities: newEntities, paths: newPaths } = parseFullMapJson(jsonData);
                ...
                <>
                    ...
                    <div className="w-64 flex flex-col border-r border-[#1a1a1a] bg-[#0f0f0f]">
                        ...
                        <div className="px-3 pt-3 pb-2">
                            <div className="flex bg-[#1a1a1a] p-1 rounded-md mb-2"><button onClick={() => setSidebarTab('assets')} className={`flex-1 py-1.5 rounded text-[10px] font-medium transition-all ${sidebarTab === 'assets' ? 'bg-[#333] text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'}`}>资源库</button><button onClick={() => setSidebarTab('layers')} className={`flex-1 py-1.5 rounded text-[10px] font-medium transition-all ${sidebarTab === 'layers' ? 'bg-[#333] text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'}`}>图层</button></div>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-3 pt-0">
                            ...
                            {sidebarTab === 'layers' && (<div className="pt-2"><div className="text-[10px] font-bold text-gray-600 uppercase mb-2 px-1 flex justify-between"><span>场景对象</span><span className="bg-[#222] px-1.5 rounded text-[9px]">{filteredObjects.length}</span></div><div className="space-y-0.5">{[...filteredObjects].reverse().map(obj => (<div key={obj.id} onClick={(e) => {
                                if (!obj.locked) {
                                    setToolMode('select');
                                    if (e.shiftKey) {
                            ...
                            <div className="flex-1 overflow-y-auto p-4">
                                <div className="mb-6">
                                    ...
                                    <div className="space-y-3 mb-6">
                                        {viewMode === '3d' && (
                                            <div className="flex items-center justify-between">
                                                <span className="text-xs text-gray-400">相机视角</span>
                                                <select
                                                    value={cameraView}
                                                    onChange={(e) => setCameraView(e.target.value)}
                                                    className="bg-[#1a1a1a] border border-[#333] rounded px-3 py-1 text-xs text-white outline-none focus:border-blue-500"
                                                >
                                        ...
                                        <div className="flex items-center justify-between">
                                            <span className="text-xs text-gray-400">网格吸附</span>
                                            <button
                                                onClick={() => setEnableSnap(!enableSnap)}
                                                className={`px-3 py-1 rounded text-xs transition-colors ${enableSnap ? 'bg-blue-600 text-white' : 'bg-[#1a1a1a] text-gray-400 hover:text-white'}`}
                                            >
                                                {enableSnap ? '开启' : '关闭'}
                                            </button>
                                        </div>
                                    ...
                                    <div className="space-y-3 mb-6">
                                        <div className="flex items-center justify-between">
                                            <span className="text-xs text-gray-400">网格大小</span>
                                            <input
                                                type="number"
                                                value={gridSize}
                                                onChange={(e) => setGridSize(Number(e.target.value))}
                                                className="w-20 bg-[#1a1a1a] border border-[#333] rounded px-2 py-1 text-xs text-white outline-none focus:border-blue-500 text-center"
                                                min="0.1"
                                                step="0.1"
                                            />
                                        </div>
                                        <div className="flex items-center justify-between">
                                            <span className="text-xs text-gray-400">网格范围</span>
                                            <span className="text-xs text-gray-400">100m × 100m</span>
                                        </div>
                                    </div>
